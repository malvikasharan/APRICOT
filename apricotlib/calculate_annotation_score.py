...
class ComputeCompositionDistance(object):
    ...
    def summarize_rps_analysis_result_files(self):
        ...
        'MolecularMass (filter-1)', 'IsoelectricPoint (filter-1)',
        'Solubility (filter-1)', 'VanDerWaalValue (filter-1)',
        'PkaValue (filter-1)', 'CoverageByDomainLength (filter-2)',
        'IdentityByCoverage (filter-2)', 'SimilarityByCoverage (filter-2)', 
        'UngappedByCoverage (filter-2)', 'AA-RODistance(filter-3)',
        'SecStr-RODistance (filter-3)', 'PhysiChemi-RODistance (filter-3)',
        'AAComp-EDistance (filter-3)', 'PhysiChemiComp-EDistance (filter-4)',
        'DipeptideComp-EDistance (filter-4)',
        'TripeptideComp-EDistance (filter-4)',
        'Average filter-1 (Chemical properties)',
        'Average filter-2 (Predicted properties (PP))',
        'Average filter-3 (Ratcliff Obershelp distance of PP)',
        'Average filter-4 (Euclidean distance of PP)'])+'\n')
        ...
        'SecStr-RODistance - P_value', 'PhysiChemi-RODistance (filter-3)',
        'PhysiChemi-RODistance -t_statistics', 'PhysiChemi-RODistance - P_value',
        'AAComp-EDistance (filter-3)', 'AAComp-EDistance - t_statistics',
        'AAComp-EDistance -P_value', 'Average filter-3 (Ratcliff Obershelp distance of PP)',
        ...
        'Ref-PhysiChemi-RODistance -t_statistics', 'Ref-PhysiChemi-RODistance - P_value',
        'Random-PhysiChemi-RODistance -t_statistics', 'Random-PhysiChemi-RODistance - P_value',
        'Ref-AAComp-EDistance - t_statistics', 'Ref-AAComp-EDistance -P_value',
        'Random-AAComp-EDistance - t_statistics', 'Random-AAComp-EDistance -P_value',
                                        ...
                                        if cdd_main in self._cdd_main_entry_dict[protein_id].keys():
                                            ...
                                            for each_stat_group in stat_data[1:]:
                                                ...
                                                query_seq, subject_seq =  self._compile_cdd_stat(
                                                    each_stat_group)
                                                seq_distance = difflib.SequenceMatcher(
                                                None, query_seq, subject_seq).ratio() 
                                                
                                                query_aac = self._compute_each_aac(query_seq)
                                                subject_aac = self._compute_each_aac(subject_seq)
                                                aac_distance = distance.euclidean(
                                                    query_aac, subject_aac)/100
                                                
                                                query_dp_comp = self._compute_dp_comp(query_seq)
                                                subject_dp_comp = self._compute_dp_comp(subject_seq)
                                                dp_comp_distance = distance.euclidean(
                                                ...
                                                tp_comp_distance = distance.euclidean(
                                                ...
                                                subject_pcn, subject_pcn_comp = self._compute_group_aac(
                                                    subject_seq)
                                                pcn_comp_distance = distance.euclidean(
                                                    query_pcn_comp, subject_pcn_comp)/100
                                                pcn_distance = difflib.SequenceMatcher(
                                                    None, query_pcn, subject_pcn).ratio()
                                                filter1 = (float(mass_score)+ float(
                                                    pi_score)+ float(solub_score)+ float(vdw_score)+
                                                    float(pka_score))/5
                                                filter2 = (coverage_percent+identity_percent+
                                                        similarity_percent+ungap_percent)/4
                                                filter3 = (float(1-seq_distance)+
                                                         float(1-ss_distance)+
                                                         float(1-pcn_distance)+
                                                         float(1-aac_distance))/4
                                                filter4 = (float(1-dp_comp_distance)+
                                                           float(1-tp_comp_distance)+
                                                         float(1-pcn_comp_distance))/3
                                                self._ref_value_dict[protein_id][domain_id]['seq_distance'] = 1-float(ss_distance)
                                                self._ref_value_dict[protein_id][domain_id]['ss_distance'] = 1-float(ss_distance)
                                                self._ref_value_dict[protein_id][domain_id]['pcn_distance'] = 1-float(pcn_distance)
                                                self._ref_value_dict[protein_id][domain_id]['aac_distance'] = 1-float(aac_distance)
                                                self._ref_value_dict[protein_id][domain_id]['dp_comp_distance'] = 1-float(dp_comp_distance)
                                                self._ref_value_dict[protein_id][domain_id]['tp_comp_distance'] = 1-float(tp_comp_distance)
                                                self._ref_value_dict[protein_id][domain_id]['pcn_comp_distance'] = 1-float(pcn_comp_distance)
                                                ...
                                                final_entry = "\t".join(map(str, [cdd_main_entry1,
                                                    ...
                                                    similarity, gaps, mass_entry, pi_entry,
                                                    ...
                                                    "%.4f" % float(1-seq_distance),
                                                    "%.4f" % float(1-ss_distance),
                                                    "%.4f" % float(1-pcn_distance),
                                                    "%.4f" % float(1-aac_distance),
                                                    "%.4f" % float(1-pcn_comp_distance),
                                                    "%.4f" % float(1-dp_comp_distance),
                                                    "%.4f" % float(1-tp_comp_distance),
                                                ...
                                                filter3_file.write('\t'.join(map(str, [protein_id,
                                                    ...
                                                    "%.4f" % float(1-pcn_distance),
                                                    self._ref_t_stat[protein_id][domain_id]['pcn_distance'],
                                                    self._ref_pval[protein_id][domain_id]['pcn_distance'],
                                                    "%.4f" % float(1-aac_distance),
                                                    self._ref_t_stat[protein_id][domain_id]['aac_distance'],
                                                    self._ref_pval[protein_id][domain_id]['aac_distance'],
                                                ...
                                                self._ref_t_stat[protein_id][domain_id]['pcn_distance'],
                                                self._ref_pval[protein_id][domain_id]['pcn_distance'],
                                                ','.join(self._t_stat_distribution[protein_id][domain_id]['pcn_distance']),
                                                ','.join(self._pval_distribution[protein_id][domain_id]['pcn_distance']),
                                                self._ref_t_stat[protein_id][domain_id]['aac_distance'],
                                                self._ref_pval[protein_id][domain_id]['aac_distance'],
                                                ','.join(self._t_stat_distribution[protein_id][domain_id]['aac_distance']),
                                                ','.join(self._pval_distribution[protein_id][domain_id]['aac_distance']),
    ...
    def _get_t_stats(self, protein_id, domain_id, query_seq, ref_subject):
        ...
        value_dict = {}
        ref_subject_set = set()
        ref_subject_set.add(ref_subject)
        query_aac = self._compute_each_aac(query_seq)
        query_ss = self._compute_sec_str(query_seq)
        query_dp_comp = self._compute_dp_comp(query_seq)
        query_tp_comp = self._compute_tp_comp(query_seq)
        ...
        value_dict.setdefault('aac_distance', []).append(
            self._ref_value_dict[protein_id][domain_id]['aac_distance'])
        ...
        for i in range(1, 501):
            if not len(list(ref_subject_set)) > 100:
                subject_list = list(ref_subject)
                random.shuffle(subject_list, random.random)
                if not ''.join(subject_list) in ref_subject_set:
                    subject_seq = ''.join(subject_list)
                    ref_subject_set.add(subject_seq)
                    seq_distance = difflib.SequenceMatcher(
                        None, query_seq, subject_seq).ratio()
                    subject_aac = self._compute_each_aac(subject_seq)
                    aac_distance = distance.euclidean(
