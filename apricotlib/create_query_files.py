...
def create_uid_query_file(uids, query_path):
    ...
    with open(query_path, 'a') as out_fh:
        if ',' in uids or not ' ' in uids:
            ...
            print(
        "Query Error Message:\nPlease check that the query UniProt ids are separated by comma\n")
        ...
        for each_gene in geneids.split(','):
            ...
            for entry in entry_dict.keys():
                if gene in entry.split('\t')[0].lower():
                    mapped_data.setdefault(each_gene, set()).add(entry_dict[entry])
                elif gene in entry.split('\t')[1].split('_')[0].lower():
                    mapped_data.setdefault(each_gene, set()).add(entry_dict[entry])
                elif gene+' ' in entry.split('\t')[3].lower() or ' '+gene in entry.split('\t')[3].lower():
                    mapped_data.setdefault(each_gene, set()).add(entry_dict[entry])
                elif gene+' ' in entry.split('\t')[4].lower() or ' '+gene in entry.split('\t')[4].lower():
                    mapped_data.setdefault(each_gene, set()).add(entry_dict[entry])
            if not gene in mapped_data.keys():
