        ...
        if os.path.isfile(inpath+'/'+csv_file):
            if csv_file.split('.')[1] in allowed_format_list:
                ...
                if len(sheet_name) > 25:
                    sheet_name = ''.join(sheet_name[:26])
                xlsxsheet = Workbook().create_sheet(0, sheet_name)
                for i, row in  enumerate(csv.reader(open(inpath+'/'+csv_file),
                                          delimiter="\t")):
                    for j, col in enumerate(row):
                        if not StrictVersion(openpyxl.__version__) >= StrictVersion('2.0.0'):
                            ...
                            if 'annotation_scoring_of_selected_data_filter' in csv_file:
                                for idx in range(1,3):
                                    if len(list(row[idx])) > 50:
                                        row[idx] = '<br>'.join(split_str(row[idx], 50))
                            else:
