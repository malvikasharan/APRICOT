...
'''summarize Literature based secondary structure result'''
...
class LiteratureSecstrAnalysis(object):
    ...
    def run_literature_ss_analysis(self):
        ...
        for files in os.listdir(self._fasta_path):
            if files.split('.')[0] in self._selected_protein_set:
                print("Literature acid based secondary structure analysis for %s" % files)
                with open(self._outpath+'/%s.plain' % files.split('.')[0], 'w') as out_fh:
                    out_fh.write("#Literature based three-class secondary structure prediction results\n\n")
                    with open("%s/%s" % (self._fasta_path, files), 'r') as in_fh:
                        literature_acid_seq = []
                        for entry in in_fh:
                        ...
                        full_seq = self._split_str(''.join(literature_acid_seq), 60)
    ...
    def create_job_completion_file(self):
        with open(self._outpath+'/literature_ss_analysis.txt', 'w') as out_fh:
            out_fh.write("Literature based secondary structures for the selected proteins are generated.\n")
